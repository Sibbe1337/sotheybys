...
export default async function PropertiesPage() {
  ...
  try {
    // Try to fetch from Linear API first - FINNISH LANGUAGE
    const linearProperties = await fetchLinearListings('fi');
  ...
  return (
    <main className="flex-1">
        ...
        <div className="container mx-auto px-4">
          <div className="py-4 flex flex-wrap gap-4 items-center justify-between">
            <div className="flex flex-wrap gap-4">
              ...
              <select className="px-4 py-2 border border-gray-300 rounded-lg font-light 
                             focus:outline-none focus:border-[#1a3a4a]">
                <option value="">Hintajärjestys</option>
                <option value="asc">Halvin ensin</option>
                <option value="desc">Kallein ensin</option>
              </select>
            </div>
            
            <p className="text-gray-600 font-light">
              {allProperties.length} kohdetta
            </p>
          </div>
        </div>
      </section>
      ...
      <section className="py-12 lg:py-20">
        <div className="container mx-auto px-4">
          <PropertyGrid properties={allProperties} showStatus={true} language="fi" />
