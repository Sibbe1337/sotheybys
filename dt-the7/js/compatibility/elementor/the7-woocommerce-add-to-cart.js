        ...
        const methods = {
                ...
                if ($variationList.length) {
                    const productVariations = new ProductVariations({
                        ...
                        render: function () {
                            ...
                            // Add the condition to make it active if length is 1
                            if ($attributeWrap.length === 1) {
                                $attributeWrap.addClass('active'); // Add 'active' class
                                $attributeWrap.trigger('click');  // Optionally trigger a click event
                            }
                    ...
                    $("li a", $variationList).on("click", function (e) {
                        ...
                        const atr = "attribute_" + $this.closest("ul").attr("data-atr");
            ...
            updateProductImage: function ($imgElement) {
                ...
                if($sliderVertical.length){
                    let layzrTimeout = null;
                    const $scroleImage = $sliderVertical.find('.dt-gallery-container img');
                    var targetSrc = $imgElement.attr('data-src');

                    var $scroleToImage = $sliderVertical.find('.dt-gallery-container img').filter(function () {
                        var attributes = this.attributes;
                        for (var i = 0; i < attributes.length; i++) {
                            var attr = attributes[i];
                            if (attr.name.includes('src') && attr.value === targetSrc) {
                                return true;
                            }
                        }
                        return false;
